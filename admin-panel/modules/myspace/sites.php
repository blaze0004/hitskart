...
if(isset($_GET['edit'])){
	...
	$sql = $db->FetchArray($db->Query("SELECT user,url,title,cpc,active FROM `myspace` WHERE `id`='".$edit."'"));
	$user = $sql['user'];
	$sit = $sql['url'];
	$s_title = $sql['title'];
	$status = $sql['active'];
	...
	if(isset($_POST['submit'])){
		$db->Query("UPDATE `myspace` SET `user`='{$_POST['user']}', `url`='{$_POST['url']}', `title`='{$_POST['title']}', `cpc`='{$_POST['cpc']}', `active`='{$_POST['status']}' WHERE `id`='".$edit."'");
		$mesaj = '<div class="alert success"><span class="icon"></span><strong>Success!</strong> Page successfully edited!</div>';
	}
}
if(isset($_GET['del']) != "" && is_numeric($_GET['del'])){$del = $db->EscapeString($_GET['del']); $db->Query("DELETE FROM `myspace` WHERE `id`='".$del."'");}
if(isset($_GET['edit'])){ 
	echo $mesaj;
?>
			...
			<div class="header">
				...
				<div class="content">
					<div class="row">
						...
						<div><input type="text" name="user" value="<?=(isset($_POST['user']) ? $_POST['user'] : $user)?>" required /></div>
					</div>
					<div class="row">
						<label><strong>URL</strong></label>
						<div><input type="text" name="url" value="<?=(isset($_POST['url']) ? $_POST['url'] : $sit)?>" required /></div>
					</div>
					<div class="row">
						<label><strong>Title</strong></label>
						<div><input type="text" name="title" value="<?=(isset($_POST['title']) ? $_POST['title'] : $s_title)?>" required /></div>
					</div>
					<div class="row">
						<label><strong>CPC</strong></label>
						<div><input type="text" name="cpc" value="<?=(isset($_POST['cpc']) ? $_POST['cpc'] : $cpc)?>" required /></div>
					</div>		
					<div class="row">
...
<?}else{?>
			<div class="box">
				<table class="styled">
					...
					<tbody>
<?
  $sql = $db->Query("SELECT * FROM `myspace`".$s_value." ORDER BY `id` ASC LIMIT ".$start.",".$limit."");
...
?>	
						<tr>
							...
							<td><a href="https://myspace.com/<?=$sites['url']?>" target="_blank">https://myspace.com/<?=$sites['url']?></a></td>
							<td><?=$rec?></td>
							<td><?=$sites['clicks']?></td>
							<td class="center">
