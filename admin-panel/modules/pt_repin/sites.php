...
$sql = $db->Query("SELECT id FROM `pt_repin`".$s_value);
...
if(isset($_GET['edit'])){
	...
	$sql = $db->FetchArray($db->Query("SELECT user,url,title,p_av,cpc,active FROM `pt_repin` WHERE `id`='".$edit."'"));
	if(isset($_POST['submit'])){
		$db->Query("UPDATE `pt_repin` SET `user`='{$_POST['user']}', `url`='{$_POST['url']}', `title`='{$_POST['title']}', `p_av`='{$_POST['p_av']}', `cpc`='{$_POST['cpc']}', `active`='{$_POST['status']}' WHERE `id`='".$edit."'");
		$mesaj = '<div class="alert success"><span class="icon"></span><strong>Success!</strong> Page successfully edited!</div>';
	}
}
if(isset($_GET['del']) != "" && is_numeric($_GET['del'])){$del = $db->EscapeString($_GET['del']); $db->Query("DELETE FROM `pt_repin` WHERE `id`='".$del."'"); $db->Query("DELETE FROM `pt_repined` WHERE `site_id`='".$del."'");}
if(isset($_GET['edit'])){ 
	echo $mesaj;
?>
...
<?}else{?>
			<div class="box">
				<table class="styled">
					...
					<tbody>
<?
  $sql = $db->Query("SELECT * FROM `pt_repin`".$s_value." ORDER BY `id` ASC LIMIT ".$start.",".$limit."");
...
?>	
						<tr>
							...
							<td><a href="http://pinterest.com/pin/<?=$sites['url']?>" target="_blank"><?=truncate($sites['url'], 60)?></a></td>
							<td><?=$rec?></td>
							<td><?=$sites['clicks']?></td>
							<td class="center">
