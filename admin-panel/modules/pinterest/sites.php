...
$sql = $db->Query("SELECT id FROM `pinterest`".$s_value);
...
if(isset($_GET['edit'])){
	...
	$sql = $db->FetchArray($db->Query("SELECT user,url,title,p_av,cpc,active FROM `pinterest` WHERE `id`='".$edit."'"));
	if(isset($_POST['submit'])){
		$db->Query("UPDATE `pinterest` SET `user`='{$_POST['user']}', `url`='{$_POST['url']}', `title`='{$_POST['title']}', `p_av`='{$_POST['p_av']}', `cpc`='{$_POST['cpc']}', `active`='{$_POST['status']}' WHERE `id`='".$edit."'");
		$mesaj = '<div class="alert success"><span class="icon"></span><strong>Success!</strong> Page successfully edited!</div>';
	}
}
if(isset($_GET['del']) != "" && is_numeric($_GET['del'])){$del = $db->EscapeString($_GET['del']); $db->Query("DELETE FROM `pinterest` WHERE `id`='".$del."'"); $db->Query("DELETE FROM `pinterested` WHERE `site_id`='".$del."'");}
if(isset($_GET['edit'])){ 
	echo $mesaj;
?>
...
<?}else{?>
			<div class="box">
				<table class="styled">
					...
					<tbody>
<?
  $sql = $db->Query("SELECT * FROM `pinterest`".$s_value." ORDER BY `id` ASC LIMIT ".$start.",".$limit."");
