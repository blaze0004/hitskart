...
$sql = $db->Query("SELECT id FROM `instagram`".$s_value);
...
if(isset($_GET['edit'])){
	...
	$sql = $db->FetchArray($db->Query("SELECT user,url,title,p_av,cpc,active FROM `yousecond` WHERE `id`='".$edit."'"));
	if(isset($_POST['submit'])){
		$db->Query("UPDATE `yousecond` SET `user`='{$_POST['user']}', `url`='{$_POST['url']}', `title`='{$_POST['title']}', `p_av`='{$_POST['p_av']}', `cpc`='{$_POST['cpc']}', `active`='{$_POST['status']}' WHERE `id`='".$edit."'");
		$mesaj = '<div class="alert success"><span class="icon"></span><strong>Success!</strong> Page successfully edited!</div>';
	}
}
if(isset($_GET['del']) != "" && is_numeric($_GET['del'])){$del = $db->EscapeString($_GET['del']); $db->Query("DELETE FROM `yousecond` WHERE `id`='".$del."'"); $db->Query("DELETE FROM `yousecond_done` WHERE `site_id`='".$del."'");}
if(isset($_GET['edit'])){ 
	echo $mesaj;
?>
			...
			<div class="header">
				...
				<div class="content">
					...
					<div class="row">
						...
						<div><input type="text" name="p_av" value="<?=(isset($_POST['p_av']) ? $_POST['p_av'] : $sql['p_av'])?>" required="required" /></div>
					</div>
					<div class="row">
...
<?}else{?>
			<div class="box">
				<table class="styled">
					...
					<tbody>
<?
  $sql = $db->Query("SELECT * FROM `yousecond`".$s_value." ORDER BY `id` ASC LIMIT ".$start.",".$limit."");
...
?>	
						<tr>
							...
							<td><a href="http://yousecond.net/<?=$sites['url']?>" target="_blank">http://yousecond.net/<?=truncate($sites['url'], 60)?></a></td>
							<td><?=$rec?></td>
							<td><?=$sites['clicks']?></td>
							<td class="center">
