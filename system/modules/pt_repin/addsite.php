...
if(empty($url)){
	...
	if(!preg_match("|^http(s)?://pinterest.com/pin/(.*)?$|i", $url)){
		$msg = '<div class="msg"><div class="error">'.$lang['repin_01'].'</div></div>';
	}else{
		$pins = @get_data($url);
		$match = array(); 
		...
		$sql = $db->Query("SELECT id FROM `pt_repin` WHERE `url`='".$pin['url']."'");
		$num = $db->GetNumRows($sql);
		if($num > 0){
			$msg = '<div class="msg"><div class="error">'.$lang['repin_02'].'</div></div>';
		}elseif($pin['url'] == '' || $pin['title'] == ''){
			$msg = '<div class="msg"><div class="error">'.$lang['repin_03'].'</div></div>';
		}else{
			$db->Query("INSERT INTO `pt_repin` (user, url, title, p_av, cpc) VALUES('".$data['id']."', '".$pin['url']."', '".$pin['title']."', '".$pin['av']."', '".$cpc."') ");
			$msg = '<div class="msg"><div class="success">'.$lang['repin_04'].'</div></div>';
			$error = 0;
		}
	}
}
