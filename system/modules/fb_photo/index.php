...
register_filter('free_coins','fbp_icon_free');
function fbp_icon_free($freeicons) {
	return $freeicons . '<th align="center"><a href="p.php?p=fb_photo"><img class="exchange_icon" src="img/icons/facebook.png" alt="Facebook" /><br/><p>Facebook Photo Likes</p></a></th>';
}
register_filter('free_coins_main','fbp_icon_main');
function fbp_icon_main($freeicons) {
	return $freeicons . '<th align="center"><a href="p.php?p=facebook"><img class="exchange_icon" src="img/icons/facebook.png" alt="Facebook" /><br/><p>Facebook Photo Likes</p></a></th>';
}
...
function fbp_stats($stats) {
	...
	if($db->GetNumRows($sql) == 0){
		...
		$db->Query("INSERT INTO `web_stats` (`module_id`,`module_name`,`value`)VALUES('fb_photo','FB Photo Likes','".$clicks."')");
	}else{
...
register_filter('tot_clicks','fbp_tot_clicks');
function fbp_tot_clicks($stats) {
	global $db;
    $clicks = $db->QueryFetchArray("SELECT value FROM `web_stats` WHERE `module_id`='fb_photo'");
    if(empty($clicks['value']) && $clicks['value'] != '0'){
		$clicks = $db->QueryFetchArray("SELECT SUM(`clicks`) AS `value` FROM `fb_photo`");
	}
...
function fbp_admin_menu($menu) {
	return $menu . '<li><a href="index.php?x=sites&s=fb_photo">FB Photo</a></li>';
}
?>