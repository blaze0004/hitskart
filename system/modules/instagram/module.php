...
$sql = $db->Query("SELECT a.id, a.inst_id, a.url, a.title, a.p_av, a.cpc, b.premium FROM instagram a LEFT JOIN users b ON b.id = a.user LEFT JOIN instagram_done c ON c.user_id = '".$data['id']."' AND c.site_id = a.id WHERE a.active = '0' AND (b.coins >= a.cpc AND a.cpc >= '2') AND (c.site_id IS NULL AND a.user !='".$data['id']."')".$dbt_value." ORDER BY a.cpc DESC, b.premium DESC".($site['mysql_random'] == 1 ? ', RAND()' : '')." LIMIT 14");
...
<script type="text/javascript">
	msg1 = '<?=mysql_escape_string($lang['inst_08'])?>';
	msg2 = '<?=mysql_escape_string($lang['inst_09'])?>';
	msg3 = '<?=mysql_escape_string($lang['inst_10'])?>';
	msg4 = '<?=mysql_escape_string($lang['inst_11'])?>';
	msg5 = '<?=mysql_escape_string($lang['inst_12'])?>';
...
<div class="follow<?=($sit['premium'] > 0 ? '_vip' : '')?>" id="<?=$sit['id']?>">
	<center>
		...
		<a href="javascript:void(0);" onclick="ModulePopup('<?=$sit['id']?>','<?=$sit['url']?>','Instagram','800','500','<?=($sit['cpc']-1)?>','1');" class="followbutton"><?=$lang['inst_05']?></a>
