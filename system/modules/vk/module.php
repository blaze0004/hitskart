...
<h2 class="title"><?=$lang['b_162']?> - VK Pages</h2>	
<div class="infobox"><div class="ucp_link active" style="margin-right:5px;display:inline-block"><a href="p.php?p=vk">VK Pages</a></div><div class="ucp_link" style="margin-right:5px;display:inline-block"><a href="p.php?p=vk_group">VK Groups</a></div></div><br />
<?
$dbt_value = '';
...
$sql = $db->Query("SELECT a.id, a.url, a.title, a.vk_photo, a.cpc, b.premium FROM vk a LEFT JOIN users b ON b.id = a.user LEFT JOIN vk_done c ON c.user_id = '".$data['id']."' AND c.site_id = a.id WHERE a.active = '0' AND (b.coins >= a.cpc AND a.cpc >= '2') AND (c.site_id IS NULL AND a.user !='".$data['id']."')".$dbt_value." ORDER BY a.cpc DESC, b.premium DESC".($site['mysql_random'] == 1 ? ', RAND()' : '')." LIMIT 14");
$sites = $db->FetchArrayAll($sql);
if($sites != FALSE){
?>
<script type="text/javascript">
	msg1 = '<?=mysql_escape_string($lang['vk_07'])?>';
	msg2 = '<?=mysql_escape_string($lang['vk_08'])?>';
	msg3 = '<?=mysql_escape_string($lang['vk_09'])?>';
	msg4 = '<?=mysql_escape_string($lang['vk_10'])?>';
	msg5 = '<?=mysql_escape_string($lang['vk_11'])?>';
...
<div class="follow<?=($sit['premium'] > 0 ? '_vip' : '')?>" id="<?=$sit['id']?>">
	<center>
		...
		<a href="javascript:void(0);" onclick="ModulePopup('<?=$sit['id']?>','<?=$sit['url']?>','VK','860','500','<?=($sit['cpc']-1)?>','1');" class="followbutton"><?=$lang['vk_12']?></a>
		<font style="font-size:0.8em;">[<a href="javascript:void(0);" onclick="skipuser('<?=$sit['id']?>','1');" style="color: #999999;font-size:0.9em;"><?=$lang['vk_14']?></a>]</font>
		<span style="position:absolute;bottom:1px;right:2px;"><a href="javascript:void(0);" onclick="report_page('<?=$sit['id']?>','<?=base64_encode('http://vk.com/'.$sit['url'])?>','vk');"><img src="img/report.png" alt="Report" title="Report" border="0" /></a></span>
	</center>
</div>
	<?}?>
</div>
